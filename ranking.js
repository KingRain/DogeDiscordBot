    ...
    client.on('message', async (message) => {
            ...
            const embed = new Discord.MessageEmbed()
                ...
                .setDescription(
                    `You've leveled up to Level: **\`${curLevel}\`**! (Points: \`${
                        Math.floor(client.points.get(key, `points`) * 100) / 100
                    }\`) `
                )
                .setColor('GREEN')
        ...
        if (message.content.toLowerCase().startsWith(`${config.prefix}rank`)) {
            ...
            for (const data of top10) {
                ...
                try {
                    i++
                    if (client.users.cache.get(data.user).tag === rankuser.tag)
                        break
                } catch {
            ...
            let curnextlevel = Number(
                (Number(1) +
                    Number(client.points.get(key, `level`).toFixed(2))) *
                    Number(10) *
                    ((Number(1) +
                        Number(client.points.get(key, `level`).toFixed(2))) *
                        Number(10))
            ...
            const rank = new canvacord.Rank()
                .setAvatar(
                    rankuser.displayAvatarURL({ dynamic: false, format: 'png' })
                )
                ...
                .setLevel(
                    Number(client.points.get(key, `level`)),
                    'Level',
                    true
                )
            ...
            for (const data of top10) {
                ...
                try {
                    ...
                    embed.addField(
                        ...
                        `Points: \`${
                            Math.floor(data.points * 100) / 100
                        }\` | Level: \`${data.level}\``
                    )
                } catch {
